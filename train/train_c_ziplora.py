...
from modules.lora import apply_ziplora, apply_retoken, ZipLoRA, ReToken
...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    class Config(TrainingCore.Config, DataCore.Config, WarpCore.Config):
        ...
        lora_checkpoint_path2: str = None
    ...
    def setup_models(self, extras: Extras) -> Models:
        ...
        trained_embedding.append(lora_checkpoint['embeddings.embeddings'].to(dtype).to(self.device))
        lora_checkpoint2 = load_or_fail(self.config.lora_checkpoint_path2)
        trained_embedding.append(lora_checkpoint2['embeddings.embeddings'].to(dtype).to(self.device))
        update_tokens = []
        for i, (tkn_regex, aggr_regex) in enumerate(self.config.train_tokens):
            if (tkn_regex.startswith('[') and tkn_regex.endswith(']')) or (tkn_regex.startswith('<') and tkn_regex.endswith('>')):
                ...
                text_model.text_model.embeddings.token_embedding.weight.data = torch.cat([
                    text_model.text_model.embeddings.token_embedding.weight.data, new_embedding + trained_embedding[i]
                ], dim=0)
        ...
        apply_ziplora(generator, filters=self.config.module_filters, rank=self.config.rank)
        text_model.text_model.to(self.device)
        generator.to(self.device)
        ziplora = nn.ModuleDict()
        # lora['embeddings'] = text_model.text_model.embeddings.token_embedding.parametrizations.weight[0]
        ziplora['weights'] = nn.ModuleList()
        for module in generator.modules():
            if isinstance(module, ZipLoRA) or (hasattr(module, '_fsdp_wrapped_module') and isinstance(module._fsdp_wrapped_module, ZipLoRA)):
                ziplora['weights'].append(module)

        lora_checkpoint1_new = {}
        for k, v in lora_checkpoint.items():
            if 'lora' in k:
                lora_checkpoint1_new[k + '1'] = v
        lora_checkpoint2_new = {}
        for k, v in lora_checkpoint2.items():
            if 'lora' in k:
                lora_checkpoint2_new[k + '2'] = v

        ziplora.load_state_dict(lora_checkpoint1_new, strict=False)
        ziplora.load_state_dict(lora_checkpoint2_new, strict=False)

        self.info.train_tokens = [(i, tokenizer.decode(i)) for i in update_tokens]
        if self.is_main_node:
            print("Updating tokens:", self.info.train_tokens)
            print(f"LoRA training {len(ziplora['weights'])} layers")

        # if self.config.lora_checkpoint_path is not None:
        #     lora_checkpoint = load_or_fail(self.config.lora_checkpoint_path)
        #     ziplora.load_state_dict(lora_checkpoint if 'state_dict' not in lora_checkpoint else lora_checkpoint['state_dict'], strict=False)

        # lora = self.load_model(lora, 'lora')
        ziplora.to(self.device).train().requires_grad_(True)
        # if self.config.use_fsdp:
        #     # fsdp_auto_wrap_policy = functools.partial(size_based_auto_wrap_policy, min_num_params=3000)
